# Product Ontology Evidence Writeback Profile

## Purpose

This profile narrows the generic `evidence-to-writeback` skill to the current `product-ontology` repository.
...
The repo-level contract in `README.md` defines:

- core evidence chain: `Source -> Artifact -> Event -> Claim -> Pattern -> Thesis`
- writeback chain: `Source -> Artifact -> Candidate -> Review/Verdict -> Writeback Intake -> Writeback`

In the current implementation, writeback generation is already stable, but the live report path is more specific:
...
Do not assume every conceptual layer is materialized as its own file. In the current report generator, some candidate and review objects are in-memory generation objects rather than durable records.
...
If you need stronger first-hand weighting, call it out explicitly instead of pretending the current generator already does it.
...
- `extra_questions`, `target_audience`, `collaboration_mode`, and preserved tensions are not decoration; they are report constraints
- writeback should be derived from approved direction plus artifact evidence, not from a title alone

## Required Output Shape

Review packs should expose:
- `Research Question`
- `Review Introduction`
...
- problem statement
- assumptions
- AI-native UX section
- current research direction
- preserved tensions

If the output falls back to placeholders such as "to be filled" or loses evidence refs, the run is not complete.

## Known Limits

- generic web discovery is still a manual-front-door workflow, not a full autonomous web ingestion stack
- `official` is a reserved adapter surface, but `fetch_official_content()` is currently unavailable; do not claim successful official ingestion when it cannot fetch content
- not every conceptual review or verdict layer is durable on disk in the current generator path
...
```bash
'/Users/vickyshou/.local/bin/uv' run --offline --with pytest python -m pytest tests/test_artifact_evidence.py -q
'/Users/vickyshou/.local/bin/uv' run --offline --with pytest python -m pytest tests/test_writeback_generate.py -q
'/Users/vickyshou/.local/bin/uv' run --offline --with pytest --with pyyaml python -m pytest tests/test_writeback_matrix.py -q
```

When editing the committed pilot writeback artifacts under `library/writebacks/podcasts/matrix/`, back up the target artifact first, then regenerate and verify against tests.
